...
class TestSimurConectividad:
    ...
    def test_cuenta_2016(self, simur_disponible):
        ...
        assert count > 50_000, (
            f"Cuenta 2016 inesperadamente baja: {count}. ¿Cambió el API?"
        )
        assert count < 100_000, (
            f"Cuenta 2016 inesperadamente alta: {count}. ¿Cambió el API?"
        )
    ...
    def test_estadisticas_por_localidad(self, simur_disponible):
        ...
        assert len(features) >= 15, (
            f"Se esperan ≥15 localidades, encontradas: {len(features)}"
        )
...
class TestSimurIntegridadDatos:
    ...
    def test_tendencia_registros_por_anio(self, simur_disponible):
        ...
        for f in features:
            ...
            n = f["attributes"]["n"]
            assert n > 40_000, (
                f"Año {anio}: {n} registros, esperados >40,000"
            )
